...
`GET /` 提供 C 排版与暮紫杏光界面，按所有者最新反馈使用低饱和烟灰紫与柔杏米色，配合宽而轻的阴影、细腻高光、半透明导航和轻微内凹输入框。静态路由只允许固定的 HTML、CSS、JS 与图标清单；不挂载仓库目录，不接受文件路径参数，也不抓取外部 URL。UI 的 CSP 仅允许同源脚本、样式、图片与 API；API 和材料仍保留独立 sandbox 策略。没有后台自动刷新。
...
运行 `python3 scripts/hub_server.py` 后在浏览器打开终端打印的本地地址。项目页提供搜索、状态和来源筛选；详情展示原始状态、完整下一步、来源位置、关联与设计入口。来源位置可复制到本机文件管理器。刷新全部仅包含当前 enabled、summary_enabled 且无管理例外的项目，单项失败保留最近成功快照。来源版本冲突时重新读取，等待用户再次发起刷新。

设计页按精确候选/基线比较本地材料，保存反馈、选择、暂缓、撤回与历史。选择与实施授权分别记录。反馈草稿和未确认命令保存在当前浏览器；它们不是权威决定。会话与 CSRF 仅在运行时内存。提交结果不确定时保持原请求 ID/载荷，明确点击重试；版本冲突保留草稿并要求重新核对。导出完成后通过按钮下载经过校验的材料。重启服务后设计历史仍来自同一持久事实库；浏览器会重新建立本地会话。
