...
DECLARATION_FIELDS = {
    "schema_version", "project_id", "source_refs", "mapping", "unknown_fields",
    "validation_entry", "max_read_age_seconds",
}
METRIC_EXPORT_FIELDS = {"entry", "snapshot"}
METRIC_SNAPSHOT_PATH = ".hub/status.json"
FIELDS = {
...
CREDENTIAL_PATH_TOKENS = {
    "auth", "authentication", "authorization", "oauth", "oauth2", "token", "tokens",
    "password", "passwords", "apikey", "key", "keys", "credential", "credentials",
    "secret", "secrets", "cookie", "cookies", "bearer",
}
CREDENTIAL_PATH_NAMES = {
    "apikey", "accesstoken", "refreshtoken", "privatekey", "serviceaccount",
    "clientsecret", "clientcredential", "idrsa", "idecdsa", "ided25519",
}
...
def credential_path_component(value: Any) -> bool:
    """One lexical credential-path deny rule shared by declarations and readers."""
    if type(value) is not str:
        return True
    folded = value.casefold()
    tokens = set(filter(None, re.split(r"[^a-z0-9]+", folded)))
    canonical = re.sub(r"[^a-z0-9]+", "", folded)
    return bool(tokens & CREDENTIAL_PATH_TOKENS
                or any(name in canonical for name in CREDENTIAL_PATH_NAMES))
...
def relative_path(value: Any) -> str:
    ...
    require(all(part not in {".", "..", "~"} and part.lower() not in FORBIDDEN_PARTS
                and not part.lower().startswith(".env") and not credential_path_component(part)
                for part in path.parts), "source path: forbidden component")
...
def validate_metric_export(value: Any) -> dict:
    """Validate one declarative Python entry, never a shell command."""
    exact(value, METRIC_EXPORT_FIELDS, "metric export")
    entry = value["entry"]
    string(entry, "metric export entry")
    ...
    require(
        ...
        and all(
            part not in {".", "..", "~"}
            and bool(ID.fullmatch(part))
            and part.lower() not in FORBIDDEN_PARTS
            and not part.lower().startswith(".env")
            and not credential_path_component(part)
            for part in path.parts
        ),
        "metric export entry must be a safe relative Python file",
    )
    relative_path(value["snapshot"])
    require(
        value["snapshot"] == METRIC_SNAPSHOT_PATH,
        "metric export must publish the canonical snapshot path",
    )
    return value
...
def validate_declaration(value: Any, project_id: str | None = None) -> dict:
    require(
        type(value) is dict
        and frozenset(value) in {
            frozenset(DECLARATION_FIELDS),
            frozenset(DECLARATION_FIELDS | {"metric_export"}),
        },
        "declaration: missing or unknown fields",
    )
    ...
    if "metric_export" in value:
        validate_metric_export(value["metric_export"])
    return value
...
def record_schema() -> dict:
    ...
    from hub.metric_management import management_contract_schema
    ...
    return {"schema_version": VERSION, "schema_language": "hub-typed-validator-v2",
            ...
            "declaration_fact_rule": "declaration contains stable source selection/mapping only; business values/status/times are generated by the resolver",
            "metric_export_contract": {
                "optional": True,
                "fields": sorted(METRIC_EXPORT_FIELDS),
                "entry_rule": "safe relative Python file; never a shell command",
                "snapshot": METRIC_SNAPSHOT_PATH,
            },
            "permission_rule": "explicit connection_read_allowed=false denies before root access; absent field falls back to enabled",
            "protected_sources": "no control/environment/credential path components in roots or sources; aliases are rechecked",
            ...
            "management_contract": management_contract_schema(),
